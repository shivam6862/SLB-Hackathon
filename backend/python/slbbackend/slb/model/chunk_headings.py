from ..model.utils import generate_response
...
def find_language(text: str) -> str:
    ...
    ans = [lang for lang in ['English', 'French',
                             'Hindi', 'Tamil'] if lang in response]
...
def find_heading_indices(text: str):
    ...
    for line in lines:
        ...
        try:
            if len(line[:line.index(':')].strip().split()) <= 3:
                ...
                assert heading == text[start:start +
                                       len(heading)], 'Error in heading extraction 000'
    ...
    lang = 'English'
    # sorting based on start index of headings
    sorted_items = sorted(headings_with_indices.items(), key=lambda x: x[1])
